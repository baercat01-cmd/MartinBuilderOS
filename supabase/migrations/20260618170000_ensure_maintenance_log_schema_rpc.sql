-- Supabase: auto-setup RPC for maintenance_log_parts / maintenance_log_documents (PIN-auth anon key).

CREATE OR REPLACE FUNCTION public.ensure_maintenance_log_schema_json(p_payload jsonb DEFAULT '{}'::jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  IF COALESCE((p_payload->>'probe_only')::boolean, false) THEN
    RETURN jsonb_build_object(
      'ok', true,
      'probe_only', true,
      'parts_table', to_regclass('public.maintenance_log_parts') IS NOT NULL,
      'documents_table', to_regclass('public.maintenance_log_documents') IS NOT NULL
    );
  END IF;

  IF to_regclass('public.maintenance_logs') IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'maintenance_logs table is missing');
  END IF;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'maintenance_log_documents_maintenance_log_part_id_fkey'
  ) THEN
    ALTER TABLE public.maintenance_log_documents
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY vehicle_documents_anon_select
      ON storage.objects FOR SELECT TO anon USING (bucket_id = 'vehicle-documents');
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY vehicle_documents_anon_insert
      ON storage.objects FOR INSERT TO anon WITH CHECK (bucket_id = 'vehicle-documents');
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE POLICY vehicle_documents_anon_delete
      ON storage.objects FOR DELETE TO anon USING (bucket_id = 'vehicle-documents');
  END IF;
...
REVOKE ALL ON FUNCTION public.ensure_maintenance_log_schema_json(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ensure_maintenance_log_schema_json(jsonb) TO anon, authenticated, service_role;
